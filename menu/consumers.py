...
#         self.group_name = f"product_group_{self.outlet}"
#         await self.channel_layer.group_add(
...
class ProductConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # # Send initial order data to the client
        # order_data = await self.get_order_data(self.outlet)
        # await self.send(text_data=json.dumps({'message': order_data}))
    ...
    async def receive(self, text_data):
        ...
        # For example, you can broadcast the message to the group
        if message:
            await self.channel_layer.group_send(
                self.group_name,
                {
    ...
    async def get_order_data(self, outlet):
        # Placeholder method to fetch order data based on outlet
        # You should implement your logic here
        return {"order": "Sample order data for outlet " + outlet}
